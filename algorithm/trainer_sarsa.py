...
def leeAlgorithm(matrix, i_start, j_start):
	...
	while len(queue) > 0:
		...
		for (x, y) in ((-1, 0), (0, -1), (0, 1), (1, 0)):
			if i + x >= 0 and i + x < n and j + y >= 0 and j + y < m and matrix[i + x][j + y] != '*' and (
			i + x, j + y) not in res:
...
class Trainer_Sarsa:
	...
	def train(self):
		...
		train_episode, additional_episodes = 0, 0
		while (train_episode < train_episodes + additional_episodes):
			if train_episode % (train_episodes // 10) == 0:
				print("Completed {} %".format(int(train_episode // (train_episodes // 10)) * 10))
			...
			maze = Maze(self.map_file_name, self.epsilon, self.distances)
			...
			if self.policy_type.lower() == "maxfirst":
				...
				raise Exception("unknown policy type. Exiting...")
				exit(-1)	
			...
			while not maze.is_final_state():
				...
				if self.policy_type.lower() == "maxfirst":
					next_curr_action = maze.maxFirst_policy(self.Q)

				elif self.policy_type.lower() == "random":
					next_curr_action = maze.random_policy(self.Q)

				elif self.policy_type.lower() == "exploatare":
					next_curr_action = maze.exploatare_policy(self.Q)

				elif self.policy_type.lower() == "explorare_exploatare":
					next_curr_action = maze.explorare_exploatare_policy(self.Q)
				...
				next_reward = self.Q.get((next_state, next_curr_action), 0)

				self.Q[(curr_state, curr_action)] = init_score + self.learning_rate * (reward + self.discount * next_reward - init_score)

				curr_state = next_state
				curr_action = next_curr_action
		...
		print(len(eval_scores), len(train_scores))
		...
		if self.final_show:
			while(True):
				...
				enter_press = True
				if enter_press:
					s = input("Press enter to continue step by step mode. Give z to resume demo or x to exit demo\n")
					if s == 'z':
						enter_press = False
					elif s == 'x':
						break
		...
		if self.plot:
			...
			plt.plot(
				np.linspace(1, train_episodes + additional_episodes, len(eval_scores)),
